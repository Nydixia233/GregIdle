...
> **冲突仲裁**：文档间内容冲突时，**设计文档 `docs/gtnh-idle-game-design.md` 是唯一事实源**，以它为准；发现其他文档与设计文档矛盾，改其他文档对齐，或先在设计文档修正再下行同步。数值结论的事实源是 VERSION.md + 源码。
...
当前默认单窗口。若开多个窗口/Agent 并行（见 [tasks/](tasks/README.md)），靠以下机制避免撞车：

- **先认领再动手**：在 [TODO.md](TODO.md) 与对应工单标注状态 + 认领窗口；认领说明任务归属，不授权两个窗口同时改同一文件。
- **文件锁**：会碰同一文件的任务，拆成不同波次或先协调文件锁，不并行写同一文件。
- **干净点开窗**：多窗口指向同一目录时共用一份文件、一个 git 暂存区。后果是 A 改了一半没提交，B `git add -A` 会卷进 A 的半成品。所以新窗口尽量在 `git status` 干净时开；各窗口只 `git add` 自己负责的文件，不 `git add -A` 扫全仓；要真并发用 git worktree（每窗口独立目录 + 分支）。
- **不擅改他人认领项**：已被其他窗口认领（🟡 + 窗口标识）的项，不并行动它，除非用户协调。
